...
// Функция записи отладочной информации в лог
...
// Парсинг аргументов командной строки: -h <ip> -p <port> -d <directory>
...
// Парсинг запроса - заполнить заголовки и вернуть файл, если получится, в теле
...
// Поток обработчика
// Фактически просто парсит запрос и отправляет ответ, завершая при этом соединение
...
// Callback для обработки полученных запросов
void read_client_cb(struct ev_loop *loop, struct ev_io *watcher, int revents) {
  // Получаем текущий watcher, чтобы через него получить доступ к информации от родителя
    struct ev_io_http *w = (struct ev_io_http *)watcher;

  // Получаем данные и проверяем их корректность
    char buf_in[CLIENT_BUF_IN_SIZE];
  ...
  // Создаем структуру для передачи потоку
...
// Сокет мастера доступен на чтение - подключился клиент
void accept_client_cb(struct ev_loop *loop, struct ev_io *watcher, int revents) {
  ...
  // Создаем watcher ввода-вывода для дочернего потока
    struct ev_io_http *w = (struct ev_io_http *)watcher;

  // Принимаем соединение, и получаем сокет для взаимодействия
    int client_sd = accept(w->io.fd, 0, 0);

  // Заполняем структуру, которую будем передавать дочернему процессу, информация для передачи:
  // директория, индекс потока, соответствующий мьютекс
  ...
  // Запускаем работу watcher'а в рамках цикла обработки событий
    ev_io_start(loop, &w_client->io);
}

// Запуск сервера: создание и настройка сокета, вотчера и цикла обработки событий
...
// Демонизация
// Форк, затем setsid и закрытие потоков ввода-вывода информации
void daemonize() {
    pid_t pid = fork();
    if (pid < 0) {
        perror("fork");
...
// Парсинг арнументов командной строки
